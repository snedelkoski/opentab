...
ACTIVATION_FUNCTIONS = {
    ...
    "rank": lambda x: np.argsort(np.argsort(x, axis=0), axis=0).astype(float)
    / (x.shape[0] - 1 + 1e-8),
}
...
class NeuralNetworkMapping:
    ...
    def __init__(self, input_dim: int, output_dim: int, n_hidden_layers: int = 1):
        ...
        for i in range(len(dims) - 1):
            ...
            activation = (
                get_random_activation() if i < len(dims) - 2 else ACTIVATION_FUNCTIONS["identity"]
            )
...
class DecisionTreeMapping:
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        """Apply the decision tree (vectorized). x: (n_samples, input_dim)"""
        n_samples = x.shape[0]
        output = np.zeros((n_samples, self.output_dim))
        self._evaluate_batch(self.tree, x, np.arange(n_samples), output)
        return output

    def _evaluate_batch(
        self, node: Dict, x: np.ndarray, indices: np.ndarray, output: np.ndarray
    ) -> None:
        """Evaluate tree on a batch of samples, writing results into *output*."""
        if len(indices) == 0:
            return
        if node["type"] == "leaf":
        ...
        self._evaluate_batch(node["left"], x, left_idx, output)
        self._evaluate_batch(node["right"], x, right_idx, output)
...
def sample_edge_mapping(input_dim: int, output_dim: int) -> Callable:
    ...
    mapping_type = random.choices(
        ["neural_network", "decision_tree", "categorical"], weights=[0.6, 0.25, 0.15]
    )[0]
    ...
    for j in range(X.shape[1]):
        if missing_mask[:, j].sum() > X.shape[0] * 0.8:
            ...
            keep_idx = np.random.choice(
                missing_idx, min(len(missing_idx), n_to_keep), replace=False
            )
    ...
    def generate(
        ...
        for node in root_nodes:
            node_values[node] = sample_initialization_data(
                ...
                hp.init_type,
                hp.init_scale,
                hp.prototype_fraction,
                hp.prototype_temperature,
            )
...
def _generate_batch_parallel(
    ...
    args_list = [
        (i, base_seed, generator_params, max_samples, max_features) for i in range(batch_size)
    ]
...
class SyntheticDatasetGenerator:
    ...
    def generate_batch(
        ...
        for i in range(batch_size):
            ...
            if dataset.missing_mask is not None:
                missing_masks[i, :n_samples, :n_features] = dataset.missing_mask[
    ...
    def generate_and_save(
        ...
        with h5py.File(output_path, "w") as f:
            ...
            f.create_dataset(
                "X",
                shape=(0, max_samples, max_features),
            ...
            f.create_dataset(
                ...
                chunks=(min(batch_size, 100), max_features),
                dtype="bool",
            )
            f.create_dataset(
                "missing_mask",
            ...
            f.create_dataset(
                "problem_type", data="regression" if self.is_regression else "classification"
            )
    ...
    for i, ax in enumerate(axes.flat):
        ...
        if n_features >= 2:
            for c in range(n_classes):
                mask = dataset.y == c
                ax.scatter(
                    dataset.X[mask, 0], dataset.X[mask, 1], alpha=0.6, label=f"Class {c}", s=20
                )
        else:
            for c in range(n_classes):
                mask = dataset.y == c
                ax.scatter(
                    dataset.X[mask, 0],
                    np.zeros_like(dataset.X[mask, 0]) + c * 0.1,
                    alpha=0.6,
                    label=f"Class {c}",
                    s=20,
                )
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", type=str, default="data/synthetic.h5", help="Output HDF5 file path"
    )
    parser.add_argument(
        "--n_datasets", "-n", type=int, default=100000, help="Number of datasets to generate"
    )
    ...
    parser.add_argument("--no-parallel", action="store_true", help="Disable parallel generation")
    parser.add_argument(
        "--visualize",
        action="store_true",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print(
            f"Sampled hyperparameters: n_nodes={params.n_nodes}, n_features={params.n_features}, "
