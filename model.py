...
class OpenTabModel(nn.Module):
    ...
    def forward(
        ...
        # Extract test sample embeddings: mean across feature columns (exclude target column)
        # The target column (last) for test positions has a dummy value (0), so
        # pooling only over real feature columns provides cleaner representations.
        test_embeddings = embeddings[:, train_size:, :-1, :].mean(dim=2)
